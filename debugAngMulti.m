function  debugAngMulti( Img, corr, xpos, ypos, disprange )
...
sno = size(corr,4);  % number of slices
S = round(sno/2);
...
for i = na:-1:1
    temp(:,:,i) = uint8(mat2gray(20*log10(abs(Img(:,:,i)./immax)+eps), [Rmin Rmax]).*255);
end
Img = temp;
cmean = squeeze(mean(corr,3));

ksize = size(squeeze(corr(:,:,1,1)));
tlc = [ypos(:), xpos(:)];
...
for i = 1:na
    im(i) = subplot(2,na+1, i); imshow(Img(:,:,i)), ...
    rectangle('Position',[tlc(S,2)-ksize(2)/2, tlc(S,1)-ksize(1)/2, ksize(2), ksize(1)],... 
    'EdgeColor','yellow')
    cim(i) = subplot(2,na+1,i+na+1); imagesc(corr(:,:,i,S), [-1 1]), axis equal tight, ...
        colormap jet
end
cave = subplot(2,na+1,[na+1,2*na+2]); imagesc(cmean(:,:,S), [-1 1]), axis equal tight, ...
        colormap jet
...
% -=< Figure resize callback function >=-
    ...
    function updateFigures()
        ...
        for i = 1:na
            set(findobj(im(i), 'Type','Rectangle'),'Position',[tlc(S,2)-ksize(2)/2, tlc(S,1)-ksize(1)/2, ksize(2), ksize(1)])
            set(findobj(cim(i), 'Type','Image'),'CData',corr(:,:,i,S))
        end
        set(findobj(cave, 'Type','Image'),'CData',cmean(:,:,S))
        grid on
    end
