function [vector] = SUBPIX2DPARAB(result_conv, interrogationarea, x, y, z, SubPixOffset)
    ...
    if(numel(x)~=0)
        ...
        for i = -1:1
            for j = -1:1
                ...
                c10(j+2,i+2, :) = i*result_conv(ip+xmax*i+j);
                c01(j+2,i+2, :) = j*result_conv(ip+xmax*i+j);
                c11(j+2,i+2, :) = i*j*result_conv(ip+xmax*i+j);
                c20(j+2,i+2, :) = (3*i^2-2)*result_conv(ip+xmax*i+j);
                c02(j+2,i+2, :) = (3*j^2-2)*result_conv(ip+xmax*i+j);
