...
def saveVectorStreams(filename, ftitle=None):
    # determine save directory
    savefilename = filename + '_pthln.mp4'
    print(savefilename)
    ...
    mask_delauny, mask_edge = vstream.makeMaskDelaunyScaled(mask, origin, xRes, yRes)  # convert mask to convex hull
    imsize = origin
    ...
    seed_x, seed_y, seed_u, seed_v = vstream.vectorstream(x, y, u, v, mask_delauny, imsize, tRes, randomize=opts.randomize,
                                                          move_along=opts.move_along, vel_weighting=opts.vel_weighting,
                                                          seed_cnt=opts.seed_count, rnd_val=opts.rnd_val,
                                                          unit_factor=unit_factor)

    seed_mag = np.sqrt(seed_u**2 + seed_v**2)

    seed_u_vis = seed_u  #seed_mag_norm*np.cos(seed_theta)
    seed_v_vis = seed_v  # seed_mag_norm*np.sin(seed_theta)

    velprc95seed = np.nanpercentile(seed_mag, 95)
    scl = (velprc95seed/np.sqrt(np.diff(x[:, 0]).mean()**2 + np.diff(y[0, :]).mean()**2))/opts.sf
    ...
    if opts.cmapmax == 'prc99':
        vcmax = np.nanpercentile(seed_mag, 99)
    elif opts.cmapmax == 'prc99.9':
        vcmax = np.nanpercentile(seed_mag, 99)
        seed99 = seed_mag[seed_mag >= vcmax]
        vcmax = np.nanpercentile(seed99, 90)
    elif isinstance(opts.cmapmax, float):
        vcmax = opts.cmapmax
    else:
        vcmax = np.nanmax(seed_mag)
    ...
    quiv = ax.quiver(seed_x[..., i], seed_y[..., i], seed_u_vis[..., i], seed_v_vis[..., i], seed_mag[..., i],
    ...
    with writer.saving(fig, savefilename, dpi=opts.dpi):
        for i in trange(u.shape[2]):
            try:
                ...
                quiv.set_UVC(seed_u_vis[..., i], seed_v_vis[..., i], seed_mag[..., i])
                quiv.set_offsets(np.vstack((seed_x[..., i], seed_y[..., i])).T)
...
if __name__ == '__main__':
    saveVectorStreams(sys.argv[1], sys.argv[2])
