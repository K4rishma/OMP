...
% Nz_start = 82;                  % Crop the image starting from Nz_start
% Nz_end = Nz_start+19;                  % Crop the image starting from Nz_start
% Nx_start = 62;
% Nx_end = Nx_start+19;


Nz_start = 43;                  % Crop the image starting from Nz_start
Nz_end = 129;                  % Crop the image starting from Nz_start
Nx_start = 46;
Nx_end = 90;
...
Nz_new = Nz_new + rem(Nz_new,2);
 ...
 %angles_check = anglesFromDerivatives(BF_filtered);
 % not a square pixel
% figure; imagesc(angles_check);
BF_new = ifft2(fftshift(fft2(BF_filtered,Nz,Nx),2),Nz_new,Nx_new);

figure; imagesc(mean(abs(BF_new),3));
%% autocorrelation
...
scan.imMask_cart = temp >40;% 40;
%figure; imshow(scan.imMask_cart);
figure; imagesc(dopV.*scan.imMask_cart);title('doppler image');
