function  debugPIV( Img, corr, xpos, ypos, disprange )
...
sno = size(corr,3);  % number of slices
S = round(sno/2);
...
for i = 1:na
    ...
    'EdgeColor','yellow')
    cim(i) = subplot(2,na+1,i+na+1); imagesc(corr(:,:,S,i), [-1 1]), axis equal tight, ...
        colormap jet
end
...
% -=< Figure resize callback function >=-
    ...
    function updateFigures()
        ...
        for i = 1:na
            set(findobj(im(i), 'Type','Rectangle'),'Position',[tlc(S,2), tlc(S,1), ksize(1), ksize(2)])
            set(findobj(cim(i), 'Type','Image'),'CData',corr(:,:,S,i))
        end
