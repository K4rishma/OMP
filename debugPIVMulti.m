function  debugPIVMulti( Img, img1_cut, img2_cut, corr, disp, xpos, ypos,spoffset, disprange,sf)
...
sno = size(img1_cut,4);  % number of slices
S = round(sno/2);
...
if ndims(Img) > 2
    Img = squeeze(Img(:,:,1));
    img1_cut = squeeze(img1_cut(:,:,1,:));
    img2_cut = squeeze(img2_cut(:,:,1,:));
end

immax = max(Img(:));

prc90 = prctile(reshape(gather(corr),[],size(corr,3)),90);

% eccentricity = zeros(size(prc90));
% for di = 1:length(prc90)
%     if ~isnan(prc90(di))
%         temp = regionprops(corr(:,:,di)>prc90(di),'Eccentricity');
%         eccentricity(di) = temp.Eccentricity;
%     end
% end
% fprintf('Mean eccentricity of correlation map is %4.2f \n', mean(eccentricity(~isnan(prc90))))
...
ksize = size(img1_cut); ksize = ksize(1:2);
tlc = [ypos(:), xpos(:)];
...
im = subplot(2,4,[1,2,5,6]); imshow(Img), ...
    rectangle('Position',[tlc(S,2)-ksize(2)/2, tlc(S,1)-ksize(1)/2, ksize(2), ksize(1)],... 
    'EdgeColor','yellow'), hold on, ...
    quiver(tlc(:,2)+spoffset, tlc(:,1)+spoffset,...
    disp(:,1).*sf, disp(:,2).*sf,0,'r','LineWidth',3);
    title(num2str(S));
im1 = subplot(2,4,3); imshow(img1_cut(:,:,S)), hold on, ...
    quiver(ksize(1)/2+spoffset,ksize(2)/2+spoffset...
    ,disp(S,1).*sf, disp(S,2).*sf,0,'r','LineWidth',3);
im2 = subplot(2,4,4); imshow(img2_cut(:,:,S));
compim = subplot(2,4,7); imshowpair(gather(img1_cut(:,:,S)),gather(img2_cut(:,:,S)));
cim = subplot(2,4,8); imagesc(corr(:,:,S), [-1 1]), axis equal tight, ...
    colormap jet, hold on, plot(ksize(1)/2+disp(S,1).*sf+spoffset,ksize(2)/2+...
    disp(S,2).*sf+spoffset,':xb')
txt1 = ['\leftarrow ', num2str(max(max(corr(:,:,S))))];
% txt2 = ['Eccentricity = ', num2str(eccentricity(S))];
txtcim = text(ksize(1)/2+double(gather(disp(S,1)))+...
    spoffset,ksize(2)/2+double(gather(disp(S,2)))+spoffset,txt1);
% txtecc = text(1,1,txt2);
grid on
...
% -=< Figure resize callback function >=-
    ...
    function updateFigures()
        set(findobj(im, 'Type','Rectangle'),'Position',[tlc(S,2)-ksize(2)/2, tlc(S,1)-ksize(1)/2, ksize(2), ksize(1)])
        set(findobj(im1, 'Type','Image'),'CData',gather(img1_cut(:,:,S)))
        try
            set(findobj(im1, 'Type','Quiver'),'UData',gather(disp(S,1).*sf))
            set(findobj(im1, 'Type','Quiver'),'VData',gather(disp(S,2).*sf))
            comptxt = '';
        catch
            comptxt = 'Complex Error';
        end
        set(findobj(im1, 'Type','Quiver'),'LineWidth',3)
        set(findobj(im2, 'Type','Image'),'CData',gather(img2_cut(:,:,S)))
        set(findobj(cim, 'Type','Image'),'CData',gather(corr(:,:,S)))
        try
            set(findobj(cim, 'Type','Line'),'XData',gather(disp(S,1))+ksize(2)/2+spoffset)
            set(findobj(cim, 'Type','Line'),'YData',gather(disp(S,2))+ksize(1)/2+spoffset)
            comptxt = '';
        catch
            comptxt = 'Complex Error';
        end
        C = imfuse(gather(img1_cut(:,:,S)),gather(img2_cut(:,:,S)));
        set(findobj(compim, 'Type', 'Image'), 'CData', C);
        txt1 = ['\leftarrow ', num2str(max(max(corr(:,:,S)))), comptxt];
%         txt2 = ['Eccentricity = ', num2str(eccentricity(S))];
        set(txtcim, 'String', txt1);
%         set(txtecc, 'String', txt2);
        set(txtcim, 'Position', [ksize(2)/2+gather(disp(S,1))+spoffset,ksize(1)/2+gather(disp(S,2))+spoffset,0])
        grid on
    end
